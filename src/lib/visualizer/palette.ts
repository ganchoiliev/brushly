...
/* Curated palette, grouped by main colour family for easy browsing. Hex values
   are approximate — calibrate against each brand's official RGB before launch
   (see brushly-visualizer-content.md). The render pipeline sends the exact hex
   as a swatch image, so these values ARE what gets painted.
   NOTE: brushly-ar/src/lib/palette.ts must remain a verbatim copy — the render
   API validates colorId/service/finish server-side; never REMOVE an id. */
...
/** Chip order in the colour chooser — light to dark, then by hue. */
...
/* Finish / sub-style options per service (drives the FinishPicker + prompt). */
